...
const ResetPassword = () => {
  ...
  const resetPass = async (data) => {
    ...
    await logoControl.start({ translateY: 100, scale: 1.66, transition: { duration: 0.5 } })
    setIsLoading(true)
    ...
    <div className="index_container">
      ...
      <main className="index_main_container">
        ...
        <motion.div className='login_container' initial={{ scaleX: 0, scaleY: 0.05 }} animate={loginContainer} exit={{ scaleY: 0 }} transition={{ default: { duration: 0.25 } }}>
          <motion.form className='login_form' onSubmit={handleSubmit(resetPass)} initial={{ opacity: 0 }} animate={loginElements} exit={{ opacity: 0 }}>
            {isEmailValid ? 
            ...
            <button className='login_item logInbtn' type='submit'>Reset Password</button>
          </motion.form>
        </motion.div>
      </main>
    </div>
